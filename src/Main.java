...
//        listGreaterThan50(entries);
    ...
    public static void printTuesdays(List<Entry> entries) {
        ...
        for (Entry entry: Entry.populate()) {
    ...
    public static void countTueWedThur(List<Entry> entries) {
        ...
        long count1 = entries.stream()
                .filter(e -> e.getDay() == Day.TUESDAY || e.getDay() == Day.THURSDAY || e.getDay() == Day.WEDNESDAY)
                .count();
    ...
    public static void weekendList(List<Entry> entries) {
        ...
        weekends = entries.stream()
                .filter(entry -> entry.getDay() == Day.SUNDAY || entry.getDay() == Day.SATURDAY)
    ...
    public static void weekdaySet(List<Entry> entries) {
        ...
        for (Entry entry: entries) {
            if(entry.getDay() == Day.TUESDAY || entry.getDay() == Day.WEDNESDAY || entry.getDay() == Day.THURSDAY
                    || entry.getDay() == Day.FRIDAY || entry.getDay() == Day.MONDAY){
                weekdays.add(entry);
            }
        ...
        weekdays = entries.stream()
                .filter(entry -> entry.getDay() == Day.MONDAY || entry.getDay() == Day.THURSDAY ||entry.getDay() == Day.FRIDAY 
                ||entry.getDay() == Day.WEDNESDAY || entry.getDay() == Day.TUESDAY)
        .collect();
        System.out.println(weekdays);
        System.out.println();
    }
